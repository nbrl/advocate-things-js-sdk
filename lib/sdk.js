;(function (context) {
     ...
     *                                 keys.
     */
    var appendKeyToUrl = function (token) {
        // For now, just assign the first sharepoint_token we find, or exit if
        // no token is given.
        // var token = sharepointData[0].sharepoint_token;
        // if (!token) {
        //   return;
        // }
    ...
    var doSend = function (data, pointType, cb) {
        ...
        xhr.onload = function () {
            ...
            if (/^20[0-9]{1}/.test(xhr.status)) {
                ...
                // FIXME: Move all of this to init's callback so we finish this function?
                //        At present, events and callbacks won't be called if we need to generate a new token. Is this right?
    ...
    var getClientToken = function() {
        var scriptElement = document.getElementById('advocate-things-script');

        // If the tag has no id set, don't attempt to find it.
        if (!scriptElement) {
            return false;
        }

        // Try to parse the client token out of the script tag.
    ...
    var init = function () {
        ...
        send(window.advocate_things_data || {}, function (err, data) {
            ...
            if (data[0].name) { // this will detect a sharepoint at present. TODO: improve
                var candidateToken = data[0].sharepoint_token;
                var re = new RegExp('[?&]' + queryParamName + '=' + candidateToken);

                // If the returned token *is* the current token, ask init for a new one.
                if (window.location.href.match(re)) {
                    init();
                } else {
                    // If it is not the same, append the returned token.
                    appendKeyToUrl(candidateToken);
                }
            }

            // Consider adding check for referred person in init() so a referral
            // can be caught on first page load - actually this might already
            // occur due to the default data send?
        });
    };
    init();
